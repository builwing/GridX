...
AGENT_REMAKER_ROOT="${AGENT_REMAKER_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
...
if [[ ! -f "$HOOKS_DIR/task-dispatcher.py" ]]; then
    # AGENT_REMAKER_ROOTからコピーを試みる
    if [[ -f "$AGENT_REMAKER_ROOT/.claude/hooks/task-dispatcher.py" ]]; then
        echo -e "${BLUE}📄 task-dispatcher.py をコピー中...${NC}"
        cp "$AGENT_REMAKER_ROOT/.claude/hooks/task-dispatcher.py" "$HOOKS_DIR/"
        echo -e "${GREEN}✅ task-dispatcher.py コピー完了${NC}"
    else
...
if [[ ! -f "$SCRIPTS_DIR/pm_auto_dispatch.sh" ]]; then
    # AGENT_REMAKER_ROOTからコピーを試みる
    if [[ -f "$AGENT_REMAKER_ROOT/scripts/core/pm_auto_dispatch.sh" ]]; then
        echo -e "${BLUE}📄 pm_auto_dispatch.sh をコピー中...${NC}"
        cp "$AGENT_REMAKER_ROOT/scripts/core/pm_auto_dispatch.sh" "$SCRIPTS_DIR/"
        echo -e "${GREEN}✅ pm_auto_dispatch.sh コピー完了${NC}"
    else
...
# generate_agents.sh
if [[ ! -f "$SCRIPTS_DIR/generate_agents.sh" ]]; then
    # AGENT_REMAKER_ROOTからコピーを試みる
    if [[ -f "$AGENT_REMAKER_ROOT/scripts/core/generate_agents.sh" ]]; then
        echo -e "${BLUE}📄 generate_agents.sh をコピー中...${NC}"
        cp "$AGENT_REMAKER_ROOT/scripts/core/generate_agents.sh" "$SCRIPTS_DIR/"
        echo -e "${GREEN}✅ generate_agents.sh コピー完了${NC}"
    else
        echo -e "${YELLOW}⚠️  generate_agents.sh が見つかりません${NC}"
        files_ok=false
    fi
else
    echo -e "${GREEN}✅ generate_agents.sh 検出${NC}"
fi

# update_pm_context7.sh
if [[ ! -f "$SCRIPTS_DIR/update_pm_context7.sh" ]]; then
    # AGENT_REMAKER_ROOTからコピーを試みる  
    if [[ -f "$AGENT_REMAKER_ROOT/scripts/core/update_pm_context7.sh" ]]; then
        echo -e "${BLUE}📄 update_pm_context7.sh をコピー中...${NC}"
        cp "$AGENT_REMAKER_ROOT/scripts/core/update_pm_context7.sh" "$SCRIPTS_DIR/"
        echo -e "${GREEN}✅ update_pm_context7.sh コピー完了${NC}"
    else
        echo -e "${YELLOW}⚠️  update_pm_context7.sh が見つかりません${NC}"
        files_ok=false
    fi
else
    echo -e "${GREEN}✅ update_pm_context7.sh 検出${NC}"
fi

# setup_pm_hooks.sh
if [[ ! -f "$SCRIPTS_DIR/setup_pm_hooks.sh" ]]; then
    # AGENT_REMAKER_ROOTからコピーを試みる  
    if [[ -f "$AGENT_REMAKER_ROOT/scripts/core/setup_pm_hooks.sh" ]]; then
        echo -e "${BLUE}📄 setup_pm_hooks.sh をコピー中...${NC}"
        cp "$AGENT_REMAKER_ROOT/scripts/core/setup_pm_hooks.sh" "$SCRIPTS_DIR/"
        echo -e "${GREEN}✅ setup_pm_hooks.sh コピー完了${NC}"
    else
        echo -e "${YELLOW}⚠️  setup_pm_hooks.sh が見つかりません${NC}"
        files_ok=false
    fi
else
    echo -e "${GREEN}✅ setup_pm_hooks.sh 検出${NC}"
fi
...
chmod +x "$SCRIPTS_DIR/update_pm_context7.sh" 2>/dev/null || true
chmod +x "$SCRIPTS_DIR/setup_pm_hooks.sh" 2>/dev/null || true
