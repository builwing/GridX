...
AGENT_REMAKER_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
...
extract_technical_specs() {
    ...
    case "$agent_name" in
        ...
        browser)
            # ブラウザ自動化とテスト仕様
            specs="MCP Playwright Server による自動化\nMCP Browser Tools による操作制御\nコンソールエラー監視\nネットワークリクエスト追跡\nパフォーマンスメトリクス取得\nクロスブラウザ対応"
            ;;
        *)
            # 汎用的な技術スタックを抽出
...
get_agent_dependencies() {
    ...
    case "$agent" in
        ...
        docs)
            ...
            ;;
        browser)
            echo "- next: フロントエンドテスト連携"
            echo "- expo: モバイルブラウザテスト連携"
            echo "- qa: E2Eテスト実行基盤"
            echo "- security: XSSやCSRF等の検証"
            echo "- pm: テスト結果報告"
            ;;
        *)
            echo "- pm: タスク管理"
...
main() {
    ...
    # スクリプトのパスを動的に解決
    local setup_pm_script=""
    if [[ -f "scripts/core/setup_pm_hooks.sh" ]]; then
        setup_pm_script="scripts/core/setup_pm_hooks.sh"
    elif [[ -f "$AGENT_REMAKER_ROOT/scripts/core/setup_pm_hooks.sh" ]]; then
        setup_pm_script="$AGENT_REMAKER_ROOT/scripts/core/setup_pm_hooks.sh"
    fi
    
    if [[ -n "$setup_pm_script" ]]; then
        chmod +x "$setup_pm_script"
        # 出力を抑制しながら終了コードを正しく取得
        "$setup_pm_script" > /dev/null 2>&1
        local pm_setup_result=$?
        if [[ $pm_setup_result -eq 0 ]]; then
    ...
    # スクリプトのパスを動的に解決
    local update_context7_script=""
    if [[ -f "scripts/core/update_pm_context7.sh" ]]; then
        update_context7_script="scripts/core/update_pm_context7.sh"
    elif [[ -f "$AGENT_REMAKER_ROOT/scripts/core/update_pm_context7.sh" ]]; then
        update_context7_script="$AGENT_REMAKER_ROOT/scripts/core/update_pm_context7.sh"
    fi
    
    if [[ -n "$update_context7_script" ]]; then
        chmod +x "$update_context7_script"
        # 出力を抑制しながら終了コードを正しく取得
        "$update_context7_script" setup > /dev/null 2>&1
        local context7_setup_result=$?
        if [[ $context7_setup_result -eq 0 ]]; then
