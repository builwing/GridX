...
extract_agents() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$in_agent_section" == true ]]; then
            ...
            # テーブル行からエージェント情報を抽出
            if [[ "$line" =~ ^\|[[:space:]]*([a-z]+)[[:space:]]*\|[[:space:]]*(.*)[[:space:]]*\|[[:space:]]*(High|Medium|Low) ]]; then
                local agent_name="${BASH_REMATCH[1]}"
                local agent_desc="${BASH_REMATCH[2]}"
                local agent_priority="${BASH_REMATCH[3]}"
                
                if [[ "$agent_name" != "エージェント名" ]]; then
...
show_summary() {
    ...
    echo "  2. 必要に応じて個別カスタマイズ"
    echo "  3. PMエージェントでタスク管理開始"
    echo ""
